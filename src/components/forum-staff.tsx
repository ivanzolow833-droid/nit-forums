...
import { ClipboardCheck, Copy, FileSignature, History, Pencil, Plus, Save, Trash2, X } from "lucide-react";
...
type StaffTab = "moderation" | "templates" | "signature" | "history";
...
export function ForumStaffPanel({ payload, onChanged }: { payload: ForumPayload; onChanged: () => Promise<void> }) {
  ...
  return <section className="space-y-5">
    ...
    {tab === "templates" ? <div className="space-y-4"><div className="admin-toolbar"><p>Переменные заполняются автоматически или запрашиваются перед отправкой. Шаблоны роли и глобальные шаблоны доступны сотрудникам только для использования, если у них нет права редактирования.</p><Button className="bg-red-600 font-bold hover:bg-red-500" onClick={() => setTemplateDraft({ ...emptyTemplate })}><Plus /> Новый шаблон</Button></div><div className="dark-panel overflow-hidden divide-y divide-white/[0.06]">{payload.templates.length ? payload.templates.map((template) => { const canEdit = template.scope === "personal" ? template.ownerId === user.id : template.scope === "role" ? user.role.permissions.includes("forum.templates.role") : user.role.permissions.includes("forum.templates.global"); return <div key={template.id} className="admin-list-row"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><strong className="text-sm text-white">{template.favorite ? "★ " : ""}{template.title}</strong><span className="private-pill">{template.scope === "personal" ? "Личный" : template.scope === "role" ? "Роли" : "Глобальный"}</span></div><p className="mt-1 truncate text-xs text-white/35">{template.body} · переменные: {template.variables.length}</p></div><div className="flex gap-2">{canEdit ? <Button size="icon-sm" variant="outline" className="admin-icon-button" onClick={() => setTemplateDraft({ ...template })}><Pencil /></Button> : null}<Button size="icon-sm" variant="outline" className="admin-icon-button" onClick={() => void perform(() => runForumAction({ action: "duplicate_template", templateId: template.id }))}><Copy /></Button>{canEdit ? <Button size="icon-sm" variant="destructive" onClick={() => { if (window.confirm(`Удалить шаблон «${template.title}»?`)) void perform(() => runForumAction({ action: "delete_template", templateId: template.id })); }}><Trash2 /></Button> : null}</div></div>; }) : <div className="empty-state">Шаблонов пока нет.</div>}</div></div> : null}

    {tab === "signature" ? <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_360px]"><div className="dark-panel space-y-4 p-5"><label className="editor-label">Текст подписи<Textarea rows={5} maxLength={500} value={signature.text} onChange={(event) => setSignature({ ...signature, text: event.target.value })} /></label><div className="grid gap-3 sm:grid-cols-2"><label className="editor-label">Цвет<Input type="color" value={signature.color} onChange={(event) => setSignature({ ...signature, color: event.target.value })} /></label><label className="editor-label">Короткий слоган<Input maxLength={120} value={signature.slogan} onChange={(event) => setSignature({ ...signature, slogan: event.target.value })} /></label></div><label className="editor-label">HTTPS-ссылка на PNG/JPG/WEBP/GIF<Input value={signature.imageUrl} onChange={(event) => setSignature({ ...signature, imageUrl: event.target.value })} placeholder="https://.../signature.gif" /></label><label className="setting-line"><input type="checkbox" checked={signature.enabled} onChange={(event) => setSignature({ ...signature, enabled: event.target.checked })} /> Автоматически добавлять после ответа</label><Button disabled={busy} className="w-full bg-red-600 font-bold hover:bg-red-500" onClick={() => void perform(() => runForumAction({ action: "save_signature", signature }))}><Save /> Сохранить подпись</Button></div><div className="dark-panel p-5"><div className="sidebar-label">Предпросмотр</div><SignaturePreview signature={signature} role={user.role.label} /></div></div> : null}
