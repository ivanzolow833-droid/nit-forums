...
const signatureColors = [
  { label: "Фиолетовый неон", value: "#a855f7" },
  { label: "Красный владельца", value: "#ff334c" },
  { label: "Золотой", value: "#f59e0b" },
  { label: "Ледяной голубой", value: "#38bdf8" },
  { label: "Розовый неон", value: "#ec4899" },
];
...
export function ForumStaffPanel({ payload, onChanged }: { payload: ForumPayload; onChanged: () => Promise<void> }) {
  ...
  return <section className="space-y-5">
    ...
    {tab === "signature" ? <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_360px]"><div className="dark-panel space-y-4 p-5"><div className="signature-gif-notice"><FileSignature /><div><strong>GIF-подписи для сотрудников</strong><span>Вставьте прямую HTTPS-ссылку на GIF — анимация появится под каждым вашим официальным ответом.</span></div></div><label className="editor-label">Текст подписи<Textarea rows={5} maxLength={500} value={signature.text} onChange={(event) => setSignature({ ...signature, text: event.target.value })} /></label><div className="grid gap-3 sm:grid-cols-2"><label className="editor-label">Цвет<div className="signature-color-picker"><Input type="color" value={signature.color} onChange={(event) => setSignature({ ...signature, color: event.target.value })} />{signatureColors.map((preset) => <button key={preset.value} type="button" className={signature.color.toLowerCase() === preset.value ? "selected" : ""} style={{ background: preset.value }} title={preset.label} aria-label={preset.label} onClick={() => setSignature({ ...signature, color: preset.value })} />)}</div></label><label className="editor-label">Короткий слоган<Input maxLength={120} value={signature.slogan} onChange={(event) => setSignature({ ...signature, slogan: event.target.value })} /></label></div><label className="editor-label">Прямая HTTPS-ссылка на PNG/JPG/WEBP/GIF<Input type="url" maxLength={1000} value={signature.imageUrl} onChange={(event) => setSignature({ ...signature, imageUrl: event.target.value })} placeholder="https://.../signature.gif" /></label><label className="setting-line"><input type="checkbox" checked={signature.autoAppend} onChange={(event) => setSignature({ ...signature, autoAppend: event.target.checked })} /> Автоматически добавлять после ответа</label><Button disabled={busy} className="w-full bg-red-600 font-bold hover:bg-red-500" onClick={() => void saveSignatureChanges()}><Save /> Сохранить подпись</Button></div><div className="dark-panel p-5"><div className="sidebar-label">Предпросмотр</div><SignaturePreview signature={signature} role={user.role.label} /></div></div> : null}
