...
export function ForumApp() {
  ...
  return <>
    ...
    {templateUse && payload.activeThread ? <TemplateVariablesDialog template={templateUse} values={templateVariables} busy={busy} onValue={(key, value) => setTemplateVariables({ ...templateVariables, [key]: value })} onClose={() => { setTemplateUse(null); setTemplateVariables({}); }} onSend={() => void perform({ action: "use_template", templateId: templateUse.id, threadId: payload.activeThread!.id, variables: templateVariables }, () => { setTemplateUse(null); setTemplateVariables({}); })} /> : null}
  </>;
}
...
function TemplateVariablesDialog({ template, values, busy, onValue, onClose, onSend }: { template: ForumTemplate; values: Record<string, string>; busy: boolean; onValue: (key: string, value: string) => void; onClose: () => void; onSend: () => void }) { const automatic = new Set(["moderator", "role", "topic_author", "date", "time", "topic_id", "topic_title", "server", "status", "appeal_link"]); const fields = template.variables.filter((variable) => !automatic.has(variable)); return <div className="editor-backdrop"><div className="dark-panel editor-modal"><div className="editor-head"><h3>{template.title}</h3><button onClick={onClose}><X /></button></div><div className="editor-content"><p className="text-sm text-white/45">Заполните переменные, которых нет в данных темы.</p>{fields.map((field) => <label key={field} className="editor-label">{field}<Input value={values[field] ?? ""} onChange={(event) => onValue(field, event.target.value)} /></label>)}<Button disabled={busy || fields.some((field) => !values[field]?.trim())} className="w-full bg-red-600" onClick={onSend}><Send /> Отправить по шаблону</Button></div></div></div>; }
