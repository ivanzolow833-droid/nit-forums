...
export function ForumApp() {
  ...
  function navigateFromRail(target: ForumNavigationTarget) {
    ...
    else if (target === "collaboration") navigate({ name: "board", boardId: "helper-apps" });
  ...
  const activeNavigation: ForumNavigationTarget = view.name === "admin" ? "admin" : view.name === "staff" ? "staff" : view.name === "users" ? "users" : view.name === "search" ? "search" : view.name === "account" ? (view.tab === "subscriptions" ? "subscriptions" : "account") : view.name === "board" ? (view.boardId === "guides" ? "guides" : view.boardId === "player-reports" ? "complaints" : view.boardId === "support" ? "support" : view.boardId === "helper-apps" ? "collaboration" : "forums") : view.name === "thread" && view.threadId === "t-rules" ? "rules" : "home";
  ...
  return <>
    ...
    <main className={`forum-page-shell ${activePreferences.sidebarCollapsed ? "navigation-collapsed" : ""} ${appearance.showRightSidebar && !["admin", "staff", "messages", "account", "search", "users"].includes(view.name) ? "with-context-sidebar" : ""}`}>
      <ForumNavigation user={user} appearance={appearance} active={activeNavigation} collapsed={activePreferences.sidebarCollapsed} mobileOpen={mobileNavigationOpen} onToggle={toggleNavigation} onCloseMobile={() => setMobileNavigationOpen(false)} onNavigate={navigateFromRail} onMarkRead={markForumRead} />
      ...
      {appearance.showRightSidebar && !["admin", "staff", "messages", "account", "search", "users"].includes(view.name) ? <ForumSidebar user={user} roles={payload.roles} members={payload.stats.members} appearance={appearance} /> : null}
    </main>
...
function AccountView({ payload, tab, busy, onTab, onThread, onAction }: { payload: ForumPayload; tab: AccountTab; busy: boolean; onTab: (tab: AccountTab) => void; onThread: (id: string) => void; onAction: (action: ForumAction) => void }) {
  ...
  return <section className="account-layout"><aside className="dark-panel account-nav"><div className="profile-summary"><Avatar user={user} /><div><strong>{user.username}</strong><RoleBadge role={user.role} /></div></div>{tabs.map((item) => <button key={item.id} className={tab === item.id ? "active" : ""} onClick={() => onTab(item.id)}>{item.label}</button>)}</aside><div className="dark-panel account-content">
    ...
    {tab === "privacy" ? <div><h2>Конфиденциальность</h2><p className="account-info">IP не показывается сотрудникам, а личные диалоги доступны только участникам. Вы сами решаете, какие признаки активности показывать возле профиля.</p><div className="preference-list"><PreferenceToggle label="Показывать статус в сети" description="Возле аватара будет видна зелёная точка." checked={preferences.showOnline} onChange={(value) => setPreferences({ ...preferences, showOnline: value })} /><PreferenceToggle label="Показывать текущую активность" description="Разрешает отображать раздел, который вы просматриваете, когда эта функция будет доступна." checked={preferences.showActivity} onChange={(value) => setPreferences({ ...preferences, showActivity: value })} /><PreferenceToggle label="Показывать подписи пользователей" description="Можно скрыть большие подписи и GIF под сообщениями." checked={preferences.showSignatures} onChange={(value) => setPreferences({ ...preferences, showSignatures: value })} /></div><Button disabled={busy} className="mt-4 bg-red-600" onClick={() => onAction({ action: "save_preferences", preferences })}><Save /> Сохранить приватность</Button></div> : null}
...
function UsersView({ users }: { users: ForumUser[] }) {
  ...
  return <section className="space-y-4"><div className="dark-panel users-heading"><div><div className="hero-kicker"><span /> Сообщество</div><h1>Пользователи форума</h1><p>Игроки и сотрудники проекта, открытые для общего списка.</p></div><div className="users-filters"><label><Search /><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Найти пользователя" /></label><select className="forum-select" value={roleId} onChange={(event) => setRoleId(event.target.value)}><option value="">Все роли</option>{roles.map((role) => <option key={role.id} value={role.id}>{role.label}</option>)}</select></div></div><div className="users-grid">{visible.map((member) => <article key={member.id} className="dark-panel user-directory-card" style={{ borderTopColor: member.preferences.profileAccent }}><div className="user-card-banner" style={{ backgroundImage: member.preferences.profileBannerUrl ? `linear-gradient(rgba(8,10,14,.35),rgba(8,10,14,.92)),url("${member.preferences.profileBannerUrl}")` : undefined }} /><div className="user-card-body"><div className="author-avatar-wrap"><Avatar user={member} />{member.preferences.showOnline ? <span className="online-dot" /> : null}</div><strong style={{ color: member.preferences.profileAccent }}>{member.username}</strong><RoleBadge role={member.role} /><span>{member.preferences.profileTitle || member.bio || "Участник сообщества CloudWorld"}</span><small>{member.postsCount} сообщений · {member.reactionsCount} реакций · {member.points} баллов</small></div></article>)}</div>{!visible.length ? <div className="dark-panel empty-state">Никого не найдено.</div> : null}</section>;
}
