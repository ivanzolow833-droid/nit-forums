...
function AuthorRail({ user }: { user: ForumUser }) { return <aside className="author-rail" style={{ backgroundImage: user.preferences.profileBannerUrl ? `linear-gradient(rgba(10,12,17,.72),rgba(10,12,17,.96)),url("${user.preferences.profileBannerUrl}")` : undefined, borderTopColor: user.preferences.profileAccent }}><div className="author-avatar-wrap"><Avatar user={user} />{user.preferences.showOnline && user.online ? <span className="online-dot" title="Сейчас на форуме" /> : null}</div><strong style={{ color: user.preferences.profileAccent }}>{user.username}</strong>{user.preferences.profileTitle ? <span className="profile-custom-title">{user.preferences.profileTitle}</span> : null}{user.role.showNearPosts ? <RoleBadge role={user.role} /> : null}{user.achievements.length ? <div className="achievement-list">{user.achievements.slice(0, 4).map((achievement) => <span key={achievement.id} title={achievement.description}>{achievement.icon} {achievement.label}</span>)}</div> : null}<small>{user.online && user.preferences.showOnline ? "Сейчас на форуме" : "Не в сети"}<br />{user.postsCount} сообщений<br />{user.points} баллов<br />Сервер: {user.preferences.serverLabel || "CloudWorld"}<br />На форуме с {formatDate(user.createdAt, true)}</small></aside>; }
