...
export function ForumApp() {
  ...
  useEffect(() => {
    ...
    const applyTheme = () => {
      ...
      const accents: Record<ForumUserPreferences["accent"], string> = { red: "#a855f7", purple: "#a855f7", blue: "#3b82f6", green: "#22c55e", orange: "#f97316" };
  ...
  const profileUser = view.name === "profile" ? [payload.currentUser, ...payload.staffUsers, payload.activeThread?.author, ...payload.posts.map((post) => post.author)].find((member) => member?.id === view.userId) ?? null : null;
  const showHero = view.name === "home" && appearance.showHero;
  const activeNavigation: ForumNavigationTarget = view.name === "admin" ? "admin" : view.name === "staff" ? "staff" : view.name === "community" ? "community" : view.name === "users" || view.name === "profile" ? "users" : view.name === "search" ? "search" : view.name === "account" ? (view.tab === "subscriptions" ? "subscriptions" : "account") : view.name === "board" ? (view.boardId === "guides" ? "guides" : view.boardId === "player-reports" ? "complaints" : view.boardId === "support" ? "support" : view.boardId === "cooperation" ? "collaboration" : "forums") : view.name === "thread" && view.threadId === "t-rules" ? "rules" : "home";
  ...
  return <>
    ...
    <main className={`forum-page-shell ${activePreferences.sidebarCollapsed ? "navigation-collapsed" : ""} ${appearance.showRightSidebar && !["admin", "staff", "messages", "account", "search", "users", "profile", "community"].includes(view.name) ? "with-context-sidebar" : ""}`}>
      ...
      {appearance.showRightSidebar && !["admin", "staff", "messages", "account", "search", "users", "profile", "community"].includes(view.name) ? <ForumSidebar user={user} roles={payload.roles} members={payload.stats.members} appearance={appearance} serverStatus={payload.serverStatus} /> : null}
    </main>
...
function UsersView({ users, onProfile }: { users: ForumUser[]; onProfile: (userId: string) => void }) {
  ...
  return <section className="space-y-4">
    ...
    <div className="users-grid">{visible.map((member) => <button type="button" key={member.id} className="dark-panel user-directory-card" style={{ borderTopColor: member.preferences.profileAccent }} onClick={() => onProfile(member.id)} title={`Открыть профиль ${member.username}`}><div className="user-card-banner" style={{ backgroundImage: member.preferences.profileBannerUrl ? `linear-gradient(rgba(8,10,14,.35),rgba(8,10,14,.92)),url("${member.preferences.profileBannerUrl}")` : undefined }} /><div className="user-card-body"><div className="author-avatar-wrap"><Avatar user={member} />{member.preferences.showOnline && member.online ? <span className="online-dot" title="Сейчас на форуме" /> : null}</div><strong style={{ color: member.preferences.profileAccent }}>{member.username}</strong><RoleBadge role={member.role} prominent /><span>{member.preferences.profileTitle || member.bio || "Участник сообщества CloudWorld"}</span><small>{member.online && member.preferences.showOnline ? "Сейчас на форуме · " : ""}{member.postsCount} сообщений · {member.reactionsCount} реакций · {member.points} баллов</small></div></button>)}</div>
    {!visible.length ? <div className="dark-panel empty-state">Никого не найдено.</div> : null}
  </section>;
}

function PublicProfileView({ user, onBack }: { user: ForumUser | null; onBack: () => void }) {
  if (!user) return <section className="dark-panel empty-state"><p>Профиль не найден или скрыт настройками роли.</p><Button className="mt-4" onClick={onBack}><ArrowLeft /> К пользователям</Button></section>;
  const accentStyle = { "--profile-color": user.preferences.profileAccent } as CSSProperties;
  return <section className="public-profile-page" style={accentStyle}>
    <button className="back-link" onClick={onBack}><ArrowLeft /> К пользователям</button>
    <div className="dark-panel public-profile-card">
      <div className="public-profile-banner" style={{ backgroundImage: user.preferences.profileBannerUrl ? `linear-gradient(100deg,rgba(7,8,14,.32),rgba(7,8,14,.94)),url("${user.preferences.profileBannerUrl}")` : undefined }} />
      <div className="public-profile-content">
        <div className="public-profile-identity"><div className="public-profile-avatar"><Avatar user={user} />{user.preferences.showOnline && user.online ? <span className="online-dot" /> : null}</div><div><div className="public-profile-name-line"><h1>{user.username}</h1>{user.preferences.showOnline && user.online ? <span>Сейчас на форуме</span> : null}</div><p>{user.preferences.profileTitle || "Участник сообщества CLOUD WORLD"}</p><RoleBadge role={user.role} prominent /></div></div>
        <div className="public-profile-stats"><div><strong>{user.postsCount.toLocaleString("ru-RU")}</strong><span>Сообщений</span></div><div><strong>{user.reactionsCount.toLocaleString("ru-RU")}</strong><span>Реакций</span></div><div><strong>{user.points.toLocaleString("ru-RU")}</strong><span>Баллов</span></div><div><strong>{user.preferences.serverLabel || "CloudWorld"}</strong><span>Сервер</span></div></div>
        <div className="public-profile-details"><article><span>О пользователе</span><p>{user.bio || "Пользователь пока ничего о себе не рассказал."}</p></article><article><span>На форуме с</span><p>{formatDate(user.createdAt, true)}</p></article></div>
        {user.achievements.length ? <div className="public-profile-achievements"><span>Достижения</span><div className="achievement-list">{user.achievements.map((achievement) => <span key={achievement.id} title={achievement.description}>{achievement.icon} {achievement.label}</span>)}</div></div> : null}
      </div>
    </div>
  </section>;
}
